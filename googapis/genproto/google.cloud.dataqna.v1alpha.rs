...
pub mod auto_suggestion_service_client {
    ...
    impl<T: Clone> Clone for AutoSuggestionServiceClient<T> {
        fn clone(&self) -> Self {
            Self {
    ...
    impl<T> std::fmt::Debug for AutoSuggestionServiceClient<T> {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            write!(f, "AutoSuggestionServiceClient {{ ... }}")
        }
    }
}
...
pub mod question_service_client {
    ...
    impl<T> std::fmt::Debug for QuestionServiceClient<T> {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            write!(f, "QuestionServiceClient {{ ... }}")
        }
    }
}
