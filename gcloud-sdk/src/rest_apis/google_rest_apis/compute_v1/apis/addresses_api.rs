...
pub struct ComputePeriodAddressesPeriodMoveParams {
    ...
    /// An optional request ID to identify requests. Specify a unique request ID so that if you must retry your request, the server will know to ignore the request if it has already been completed. For example, consider a situation where you make an initial request and the request times out. If you make the request again with the same request ID, the server can check if original operation with the same request ID was received, and if so, will ignore the second request. This prevents clients from accidentally creating duplicate commitments. The request ID must be a valid UUID with the exception that zero UUID is not supported ( 00000000-0000-0000-0000-000000000000).
    pub request_id: Option<String>,
    pub region_addresses_move_request: Option<models::RegionAddressesMoveRequest>,
}
...
) -> Result<models::AddressAggregatedList, Error<ComputePeriodAddressesPeriodAggregatedListError>> {
...
) -> Result<models::Operation, Error<ComputePeriodAddressesPeriodDeleteError>> {
    let local_var_configuration = configuration;
...
) -> Result<models::Address, Error<ComputePeriodAddressesPeriodGetError>> {
    let local_var_configuration = configuration;
...
) -> Result<models::Operation, Error<ComputePeriodAddressesPeriodInsertError>> {
    let local_var_configuration = configuration;
...
) -> Result<models::AddressList, Error<ComputePeriodAddressesPeriodListError>> {
    let local_var_configuration = configuration;
...
) -> Result<models::Operation, Error<ComputePeriodAddressesPeriodMoveError>> {
    let local_var_configuration = configuration;
...
) -> Result<models::Operation, Error<ComputePeriodAddressesPeriodSetLabelsError>> {
    let local_var_configuration = configuration;
