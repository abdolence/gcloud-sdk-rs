...
pub struct EvaluatedSlice {
    ...
    /// be in the `\[detectionPointForecast - expectedDeviation,  detectionPointForecast + expectedDeviation\]` range. The more the actual
...
pub struct SlicingParams {
    ...
    /// following pinned dimensions. A query with a pinned dimension `{ name: "d3"  stringVal: "v3" }` will only analyze events which contain the dimension `{  name: "d3" stringVal: "v3" }`.
